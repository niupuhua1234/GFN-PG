...
class SubTrajectoryBalance(TrajectoryDecomposableLoss):
    ...
    def __call__(self, trajectories: Trajectories) -> LossTensor:
        ...
        if self.weighing == "equal_within":
            ...
            contributions = torch.ones(n_rows, len(trajectories),device=trajectories.actions.device) / n_sub_trajectories
            ...
            contributions = (self.lamda ** torch.arange(trajectories.max_length,device=trajectories.actions.device).double()).float()
            contributions = contributions.unsqueeze(-1).repeat(1, len(trajectories))
            contributions = contributions.repeat_interleave(torch.arange(trajectories.max_length, 0, -1,device=trajectories.actions.device),dim=0,
