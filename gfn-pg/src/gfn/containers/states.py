...
class States(Container, ABC):
    ...
    def update_masks(self,actions:Optional[TensorLong],index:Optional[TensorLong]):# -> None:
    ...
    def extend_with_sf(self, required_first_dim: int) -> None:
        ...
        if len(self.batch_shape) == 2:
            ...
            if self.forward_masks is not None and self.backward_masks is not None:
                self.forward_masks = torch.cat((self.forward_masks,
                                                torch.ones(padding_shape,*self.forward_masks.shape[1:],dtype=torch.bool,device=self.device)),dim=0)
                self.backward_masks = torch.cat((self.backward_masks,
                                                 torch.ones(padding_shape,*self.backward_masks.shape[1:],dtype=torch.bool,device=self.device)),dim=0)
